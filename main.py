...
def generar_reporte_excel(datos):
    ...
    for componente, d in zip(procesar_componentes.__code__.co_consts[1], datos):
        fila = [
            componente,
    ...
    ahora = datetime.now()
    nombre_archivo = f"reporte_{ahora.strftime('%Y%m%d%H%M')}.xlsx"   
    print(f"Reporte generado y guardado en {nombre_archivo}")
...
def actualizar_google_sheets(datos):
    ...
    nueva_hoja = sheet.add_worksheet(title=fecha_actual, rows="100", cols="10")
    
    headers = list(datos.keys())
    valores = list(datos.values())
    nueva_hoja.append_row(headers)
    nueva_hoja.append_row(valores)
    return fecha_actual

def enviar_correo(fecha):
    remitente = "tu_correo@empresa.com"
    destinatario = "destinatario@empresa.com"
    asunto = f"Reporte SonarQube - {fecha}"
    cuerpo = f"Se ha generado un nuevo reporte en la hoja de cálculo con fecha {fecha}"
    ...
    with smtplib.SMTP("smtp.office365.com", 587) as server:
        server.starttls()
        server.login("tu_correo@empresa.com", "TU_CONTRASEÑA")
        server.sendmail(remitente, destinatario, mensaje.as_string())
    
    print("Correo enviado exitosamente.")

def procesar_componentes():
    componentes = [
    ...
    for c in componentes:
        datos = obtener_datos_sonarqube(c)
        if datos:
...
if __name__ == "__main__":
    datos = procesar_componentes()
    print("Datos obtenidos:")
    for d in datos:
        print(d)
    generar_reporte_excel(datos)
    #if datos:
        #fecha = actualizar_google_sheets(datos)
        #enviar_correo(fecha)
